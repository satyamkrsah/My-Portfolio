        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex-shrink-0 group cursor-pointer">
            <motion.div
              ...
              className="relative"
            >
              <span className="text-2xl font-black bg-gradient-to-r from-blue-600 via-purple-600 to-cyan-500 bg-clip-text text-transparent tracking-tight group-hover:from-blue-500 group-hover:via-purple-500 group-hover:to-cyan-400 transition-all duration-300">
                Satyam.dev
              </span>
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map((item) => {
              const isActive = activeSection === (item.path === '/' ? 'hero' : item.path.replace('/', ''));
              return (
                <Link
                  key={item.path}
                  to={item.path}
