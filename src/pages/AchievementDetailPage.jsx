  ...
  if (!achievement) {
    return (
      <div className="min-h-screen flex items-center justify-center dark:bg-slate-950 dark:text-white">
        ...
        <button onClick={() => navigate('/achievements')} className="ml-4 text-blue-500 underline">Go Back</button>
      </div>
    );
  }
          ...
          >
            ...
            <div className="text-center mb-10">
              ...
              <div className="flex flex-wrap justify-center gap-4 text-sm font-medium text-slate-600 dark:text-slate-400">
                <span className="flex items-center gap-1"><FiCalendar /> {achievement.date}</span>
                <span className="flex items-center gap-1"><FiTag /> {achievement.category}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
              <div className="md:col-span-2 space-y-4">
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-2">Overview</h3>
                <p className="text-lg leading-relaxed text-slate-700 dark:text-slate-300">
                  {achievement.description}
                </p>
              </div>
              <div className="space-y-4">
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-2">Technologies / Skills</h3>
                <ul className="flex flex-wrap gap-2">
                  {achievement.skills.map((skill, idx) => (
                    <li key={idx} className="flex items-center gap-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-3 py-1.5 rounded-full text-sm font-semibold border border-blue-200 dark:border-blue-800/50">
                      <FiTool /> {skill}
                    </li>
                  ))}
                </ul>
            ...
            {achievement.gallery && achievement.gallery.length > 0 && (
              <div>
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-2 mb-6">Gallery</h3>
